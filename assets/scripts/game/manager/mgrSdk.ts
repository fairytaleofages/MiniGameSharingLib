...
export default class mgrSdk extends Manager {
    ...
    public static onMsgNativeCallback(e) {
        ...
        switch (cmd) {
            ...
            case "/c/getLoginUserMessage": {
                ...
                mgrRecord.setUserId( data.userId );
                break;
            }
            ...
            /** 拉取存档结果 */
            case "/c/getUserDataResult": {
                console.log("/c/getUserDataResult");
                let data = responseData.data || {};
                mgrRecord.onReceivedServerStorage(data.userData)
                break;
            }

            /** 保存存档结果 */
            case "/c/saveUserDataResult": {
                console.log("/c/saveUserDataResult");
                break;
            }   

             /** cdk结果 */
             case "/c/useCdkey": {
                console.log("/c/useCdkey");
                mgrTip.closeWaitingDialog();
                let data = responseData.data || {};
                if ( parseInt(data.code) == 1 ) {
                    console.info("领取 成功 ");
                    let cdkData = data.data || {};
                    for (const key in cdkData) {
                        const v = cdkData[key];
                        mgrPlayer.addItemAmount( v.goodsid, v.count, "CDK领取" );
                        mgrTip.addGotItemTip( v.goodsid, v.count );
                    }
                } else {
                    console.info("领取 失败 msg: ", data.data);
                    mgrTip.showMsgTip(data.data || "兑换码领取失败");
                }
                break;
            }
    ...
    /** 保存用户存档 */
    public static saveServerRecord ( data: string ) {
    ...
    /** 试用cdk */
    public static useCdk(cdk) {
        console.log("mgrSdk useCdk cdk: ", cdk );
        if (mgrTip.isWaitingDialogOpened()) {
            mgrTip.showMsgTip("领取中，请稍后再领取")
            return;
        }
